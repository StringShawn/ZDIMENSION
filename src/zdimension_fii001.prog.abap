...
data: wa_zdimscnro like zdimscnro.
*-------------借贷标识帮助内表-----------------*
...
at SELECTION-SCREEN on VALUE-REQUEST FOR p_hkont.
  perform frm_help_hkont using p_hkont.
  "权限检查
AT SELECTION-SCREEN.
...
**               ID 'EKORG' FIELD '*'
  ...
  SHIFT p_hkont RIGHT DELETING TRAILING space.
...
* 检查会计科目
  CLEAR ska1.
  SELECT SINGLE xspeb FROM ska1 INTO ska1-xspeb WHERE saknr = p_hkont.
  IF ska1-xspeb = 'X'.
    MESSAGE e888(sabapdocu) WITH '会计科目已冻结，不能上传银行对帐单！'
.
  ENDIF.
  SELECT SINGLE txt50 FROM skat INTO g_txt50 WHERE saknr = p_hkont.
  ...
  if i_flag eq 'X'.
    SELECT  bsis~bukrs
      ...
      FROM bsis  inner join bkpf on bsis~bukrs = bkpf~bukrs
        ...
        WHERE bsis~bukrs = p_bukrs AND
              bsis~hkont = p_hkont AND
              bsis~gjahr = p_gjahr AND
              bsis~monat = p_monat
              "and bkpf~RLDNR = zdimldgrp-RLDNR
              .
    ...
    SELECT  bsas~bukrs
         ...
         FROM bsas  inner join bkpf on bsas~bukrs = bkpf~bukrs
                                       ...
                                       and bsas~BELNR = bkpf~belnr                               "已结算会计凭证
        APPENDING CORRESPONDING FIELDS OF TABLE it_bsis
        WHERE bsas~bukrs = p_bukrs AND
              bsas~hkont = p_hkont AND
              bsas~gjahr = p_gjahr AND
              bsas~monat = p_monat
              "and bkpf~RLDNR = zdimldgrp-RLDNR
            .
    loop at it_bsis ASSIGNING <fs_bsis>.
      <fs_bsis>-waers = t001-waers.
    endloop.
  else.
    SELECT  bsis~bukrs
            bsis~hkont
...
*过滤冲销凭证
  SELECT * FROM bkpf INTO CORRESPONDING FIELDS OF TABLE d_bkpf
    ...
    AND   stblg <> ''
    AND   stjah <> ''
    "and  RLDNR = zdimldgrp-RLDNR
    .
  SORT d_bkpf BY bukrs stjah stblg.
  LOOP AT it_bsis.
    READ TABLE d_bkpf WITH KEY bukrs = it_bsis-bukrs
                               stjah = it_bsis-gjahr
                               stblg = it_bsis-belnr
                               BINARY SEARCH.
    IF sy-subrc = 0.
...
*科目与银行帐号及模板的关系
  ...
  if sy-subrc ne 0.
    ...
    STOP.
  elseif ZDiMBUHK-ZDIMBTNO is initial.
...
*检查银行帐号：
    if ZDIMWKPF-ZDIMUSCOD ne ZDiMBUHK-ZDIMUSCOD.
...
*----------------------------------------------------------------------*
FORM datainputcheck .
  "changed by d00194 for all screen 0100 to 9000 at 090527
